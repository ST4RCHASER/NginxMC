                        ...
                        new Runnable() {
                            ...
                            public void run() {
                                ...
                                if (player.getServer().getInfo().getName().equalsIgnoreCase("lobbymaster")) {
                                    ...
                                    try {
                                        ResultSet sender_result_set = core.getGetBungeeConn().createStatement().executeQuery("SELECT * FROM `players` WHERE `username` LIKE '" + sender.getName() + "'");
                                        sender_result_set.next();
                                        int sender_class = core.getClassID((ProxiedPlayer) sender);
                                        if (sender_class < 6) {
                                        ...
                                        if (args[0].equalsIgnoreCase("wp")) {
                                            target_resultset = core.getGetBungeeConn().createStatement().executeQuery("SELECT * FROM `players` WHERE `username` LIKE '" + args[1] + "'");
                                            ...
                                            give_wp(args[1], Integer.parseInt(args[2]), res_str);
                                            return;
                                        }

                                        target_resultset = core.getGetBungeeConn().createStatement().executeQuery("SELECT * FROM `players` WHERE `username` LIKE '" + args[0] + "'");
                                        if (!target_resultset.isBeforeFirst()) {
                                    ...
                                    onewp.setHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, (new ComponentBuilder("§bทำการเตือนผู้เล่นแบบเริ่มต้น เหมาะสำหรับผู้เล่นที่ ฟลัตแชท ใช้โปรแกรมโกงเล็กน้อย")).create()));
                                    ...
                                    twowp.setHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, (new ComponentBuilder("§bทำการเตือนผู้เล่นแบบระดับกลาง เหมาะสำหรับผู้เล่นที่ ฟลัตแชทแล้วไม่ฟัง ใช้โปรแกรมโกงที่ทำให้เสียสมุดุล")).create()));
                                    ...
                                    treewp.setHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, (new ComponentBuilder("§bผู้เล่นจะถูกระงับการเล่นทันที แต่ยังจะสามารถพิมพ์แชทใน lobby ได้")).create()));
                                    ...
                                    four.setHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, (new ComponentBuilder("§bผู้เล่นจะถูกระงับการเล่นทันที และจะไม่สามารถพิมพ์แชทใน lobby ได้ เหมาะสำหรับผู้เล่นที่ โปรโมทเซิร์ฟเวอร์")).create()));
                                    ...
                                    kick.setHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, (new ComponentBuilder("§bผู้เล่นจะติตปีกออกจากเซิร์ฟเวอร์ทันที เมื่อกด")).create()));
                                    ...
                                    sender.sendMessage("§bวิธีการใช้งาน ให้นำเมาส์ไปวาง หรือ คลิ๊กเพื่อทำการให้ warnpoint");
                        ...
                        new Runnable() {
                            ...
                            public void run() {
                                try {
                                    ...
                                    ResultSet target_res = core.getGetBungeeConn().createStatement().executeQuery("SELECT * FROM `players` WHERE `username` LIKE '" + name + "'");
                                    target_res.next();
                                    ...
                                    core.getGetBungeeConn().createStatement().executeUpdate("UPDATE `nginxmc`.`players` SET `wp` = '" + new_wp + "' WHERE `players`.`id` = " + id + ";");
                                    if (new_wp == 3) {
