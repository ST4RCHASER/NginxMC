                ...
                .runAsync(core.getBungeeDeluxe,
                        new Runnable() {
                            ...
                            public void run() {
                                ...
                                try {
                                    sender_class = core.getClassID((ProxiedPlayer) sender);
                                ...
                                if (player.getServer().getInfo().getName().equalsIgnoreCase("lobbymaster")) {
                                    ...
                                    if (args[0].equalsIgnoreCase("admin")) {
                                        if (sender_class < 6) {
                                            player.sendMessage(ChatColor.GRAY + "Nginx: " + ChatColor.RED + "คำสั่ง /report admin ใช้ได้เฉพาะทีมงานเท่านั้น");
                                            return;
                                        }

                                        if (args[1].equalsIgnoreCase("teleport_server_staff_lkasdlksajkd") && sender_class >= 6) {
                                            core.sendToServer((ProxiedPlayer)sender, getInstance().getServerInfo(args[2]));
                                            return;
                                        }
                                        ...
                                        if (args[1].equalsIgnoreCase("claim")) {
                                            if (args.length > 3) {
                                                try {
                                                    statement = core.getGetBungeeConn().createStatement();
                                                    ...
                                                    statement.executeUpdate("UPDATE `nginxmc`.`report_log` SET `status` = '1', `end_by` = '" + sender.getName() + "', `result` = '" + final_res + "' WHERE `report_log`.`id` = '" + args[2] + "';");
                                            ...
                                            try {
                                                ...
                                                if (rowCount < 1) {
                                                    ...
                                                    do {
                                                        ...
                                                        String sender_sv_display = res.getString("sender_sv");
                                                        String target_sv_display = res.getString("target_sv");
                                                        newtime = arg.replaceAll("-", "/");
                                                        sender.sendMessage(ChatColor.AQUA + "[ID:" + id + "] " + ChatColor.WHITE + "[" + newtime + "]");
                                                        sender.sendMessage(ChatColor.GRAY + "รายงานโดย: §f[§e" + sender_sv_display + "§f] " + ChatColor.GREEN + sender_v);
                                                        sender.sendMessage(ChatColor.GRAY + "ผู้ถูกรายงาน: §f[§e" + target_sv_display +"§f] " + ChatColor.GREEN + target_v);
                                                        sender.sendMessage(ChatColor.GRAY + "เหตุผลที่รายงาน: " + ChatColor.GREEN + reason);
                                                        ProxiedPlayer target_p = getInstance().getPlayer(target_v);
                                        ...
                                        try {
                                            ResultSet resultSet = core.getGetBungeeConn().createStatement().executeQuery("SELECT * FROM `players` WHERE `username` LIKE '" + args[0] + "'");
                                        ...
                                        ProxiedPlayer player_target = getInstance().getPlayer(args[0]);
                                        if (player_target == null) {
                                            target_class = 0;
                                        }else {
                                            target_class = core.getClassID(player_target);
                                        }
                                        if (target_class >= 6) {
                                        ...
                                        try {
                                            ...
                                            String target_sv = "#OFFLINE#";
                                            if (getInstance().getPlayer(target_v) != null) {
                                                target_sv = getInstance().getPlayer(target_v).getServer().getInfo().getName();
                                            }
                                            statement.executeUpdate("INSERT INTO `nginxmc`.`report_log` (`time`, `sender`, `target`, `reason`, `status`, `sender_sv`, `target_sv`) VALUES (CURRENT_TIME, '"+sender.getName()+"', '"+args[0]+"', '"+newtime+"', '0', '"+((ProxiedPlayer) sender).getServer().getInfo().getName()+"', '"+target_sv+"');");
                                            ...
                                            String sender_sv_display = res.getString("sender_sv");
                                            String target_sv_display = res.getString("target_sv");
                                            for (ProxiedPlayer pp : getInstance().getPlayers()) {
                                                ProxiedPlayer p = pp;
                                                if (core.getClassID(pp) >= 6) {
                                                    ProxiedPlayer target_p = getInstance().getPlayer(target_v);
                                                    ...
                                                    p.sendMessage(ChatColor.GRAY + "รายงานโดย: §f[§e" + sender_sv_display + "§f] " + ChatColor.GREEN + sender_v);
                                                    p.sendMessage(ChatColor.GRAY + "ผู้ถูกรายงาน: §f[§e" + target_sv_display +"§f] " + ChatColor.GREEN + target_v);
